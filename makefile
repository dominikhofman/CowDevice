...
#	sudo python3 dynamictool.py $(MAC) --password $(PASSWORD) -l 1 -t 04:23:55 -d 10/10/20 -p 4 -a 250 -s 2 $(VERBOSITY)
	sudo python3 dynamictool.py $(MAC) --password $(PASSWORD) -s 1 $(VERBOSITY)
...
mass:
	python3 ./masstool.py --devices-to-process ./devices.txt --devices-processed ./devices_done.txt --timeout 30 \
		--cmd "python3 /home/pi/CowDeviceMenager/dynamictool.py {mac} --led 1 -vvv"
		# --cmd "python3 /home/pi/CowDeviceMenager/dynamictool.py {mac} -r -vvv"
#		--cmd "python3 /home/pi/CowDeviceMenager/dynamictool.py {mac} --password .Mafiozo#102. -s 1 -vvv"
	# --cmd "bash /home/pi/CowDeviceMenager/device_cmd.sh {mac}"